...
#     @classmethod
#     def _on_reload(cls):
#         async def _on_reload_async():
#             cls._button_event(False)
#             await AsyncUtils.setUp()
#             cls._init_button_event()
#         asyncio.ensure_future(_on_reload_async())
#         return
    
#     @classmethod
#     def _on_reset(cls,callback_fn=None):
#         async def _on_reset_async():
#             cls._button_event(False)
#             await AsyncUtils.reset_async()
#             cls._init_button_event()
#             if (callback_fn is not None):
#                 callback_fn()
...
#         cls.sub_id = input_interface.subscribe_to_keyboard_events(keyboard, cls._on_input)
...
#         input_interface = carb.input.acquire_input_interface()
#         keyboard = app_window.get_keyboard()
#         input_interface.unsubscribe_to_keyboard_events(keyboard,cls.sub_id)

#     @classmethod # TODO refactor later maybe
#     def _on_input(cls,event):
#         if event.input == carb.input.KeyboardInput.SPACE:
#             if event.type == carb.input.KeyboardEventType.KEY_PRESS:
#                 if not AsyncUtils._sample._world.is_playing():
#                     cls._on_simulation_play_pause(True)
#                 else:
#                     cls._on_simulation_play_pause(False)
#         if event.input == carb.input.KeyboardInput.J:
#             if event.type == carb.input.KeyboardEventType.KEY_PRESS:
#                 cls._on_gripper_action(True)
#         if event.input == carb.input.KeyboardInput.K:
#             if event.type == carb.input.KeyboardEventType.KEY_PRESS:
#                 cls._on_gripper_action(False)
          
#     # def on_space_pressed(event):
#     #     if event.type == carb.input.KEY_PRESS_EVENT and event.payload['key'] == carb.input.Keyboard.SPACE:
#     #         print("Space bar pressed!")

#     @classmethod
#     def _button_event(cls,flag):
#         cls.ui_elements["Simulation"].enabled = flag
#         cls.ui_elements["Follow Target"].enabled = flag
#         cls.ui_elements["Start Logging"].enabled = flag
#         cls.ui_elements["Gripper Action"].enabled = flag
#         cls.ui_elements["(Re)load"].enabled = flag
#         cls.ui_elements["Clear"].enabled = flag
#         cls.ui_elements["Stop(Reset)"].enabled = flag
#         # AttributeError: 'omni.ui._ui.SimpleStringModel' object has no attribute 'enabled'
#         # cls.ui_elements["Output Directory"].enabled = flag
#         cls.ui_elements["Save Data"].enabled = flag

#     @classmethod
#     def _init_button_event(cls):
#         cls._button_event(True)
#         #if cls.ui_elements["Follow Target"].text == "TOSTOP":
#         cls.ui_elements["Follow Target"].text = "TOSTART"
#         # if cls.ui_elements["Simulation"].text == "TOPAUSE":
#         cls.ui_elements["Simulation"].text = "TOPLAY"
#         # if cls.ui_elements["Gripper Action"].text == "TOCLOSE":
#         cls.ui_elements["Gripper Action"].text = "TOOPEN"

#         cls.ui_elements["Start Logging"].text = "START"
#         cls.ui_elements["(Re)load"].text = "(Re)load"
#         cls.ui_elements["Clear"].text = "Clear"
#         cls.ui_elements["Stop(Reset)"].text = "Stop(Reset)"
#         cls.ui_elements["Save Data"].text = "Save Data"
#         return
    
#     @classmethod # TODO maybe refactor using publisher subscriber/callback
#     def _on_simulation_play_pause(cls, val):
#         asyncio.ensure_future(AsyncUtils._sample._on_simulation_play_pause_async(val))
#         if not val:
#             cls.ui_elements["Follow Target"].text = "TOSTART"
#             cls.ui_elements["Simulation"].text = "TOPLAY"
#             asyncio.ensure_future(AsyncUtils._sample._on_follow_target_event_async(False))
#         else:
#             cls.ui_elements["Simulation"].text = "TOPAUSE"
#         return
    
#     @classmethod # TODO maybe refactor using publisher subscriber/callback
#     def _on_follow_target_button_event(cls, val):
#         if val:
#             cls.ui_elements["Simulation"].text = "TOPAUSE"
#         else:
#             cls.ui_elements["Simulation"].text = "TOPLAY"
#         asyncio.ensure_future(AsyncUtils._sample._on_follow_target_event_async(val))
#         return
    
#     @classmethod
#     def _on_gripper_action(cls, val):
#         asyncio.ensure_future(AsyncUtils._sample._on_gripper_action_async(val))
#         if not val:
#             cls.ui_elements["Gripper Action"].text = "TOOPEN"
#         else:
#             cls.ui_elements["Gripper Action"].text = "TOCLOSE"            
#         return

#     @classmethod
#     def _on_logging_button_event(cls, val):
#         AsyncUtils._sample._on_logging_event(val)
#         cls.ui_elements["Save Data"].enabled = True
#         return
    
#     @classmethod
#     def _on_save_data_button_event(self):
#         AsyncUtils._sample._on_save_data_event(self.ui_elements["Output Directory"].get_value_as_string())
#         return
    
#     @classmethod
#     def _add_buttons(cls):
#         with cls.ui_window.frame:
#             with omni.ui.VStack(): # TODO refactor later...
#                 dict = {
#                     "label": "(Re)load",
#                     "type": "button",
#                     "text": "(Re)load",
#                     "tooltip": "(Re)load Scene",
#                     "on_clicked_fn":cls._on_reload,
#                 }
#                 cls.ui_elements["(Re)load"] = btn_builder(**dict)
...
#                     "text": "Clear",
#                     "tooltip": "Clear Scene",
#                     "on_clicked_fn":lambda:asyncio.ensure_future(AsyncUtils.tearDown()),
#                 }
#                 cls.ui_elements["Clear"] = btn_builder(**dict)
...
#                     "on_clicked_fn": cls._on_reset,
#                 }
#                 cls.ui_elements["Stop(Reset)"] = btn_builder(**dict)
                


#                 }
#                 cls.ui_elements["Follow Target"] = state_btn_builder(**dict)
...
#                     "a_text": "TOOPEN",
#                     "b_text": "TOCLOSE",
#                     "tooltip": "Gripper Action",
#                     "on_clicked_fn":cls._on_gripper_action,
#                 }
#                 cls.ui_elements["Gripper Action"] = state_btn_builder(**dict)
...
#                     "read_only": False,
#                 }
#                 cls.ui_elements["Output Directory"] = str_builder(**dict)
...
#                     "on_clicked_fn": cls._on_logging_button_event,
#                 }
#                 cls.ui_elements["Start Logging"] = state_btn_builder(**dict)
...
#                     "text": "Save Data",
#                     "tooltip": "Save Data",
#                     "on_clicked_fn": cls._on_save_data_button_event,
#                 }
#                 cls.ui_elements["Save Data"] = btn_builder(**dict)
            
#     @classmethod
#     def setUp(cls, window_name: str = "HelloWorld") -> None:
#         cls.bind_inputs()
...
#     @classmethod
#     def tearDown(cls,window_name: str = "HelloWorld") -> None:
#         asyncio.ensure_future(AsyncUtils.tearDown())

#         """Destructor for the window."""
#         # destroy the window
#         try: # thus also works with no input bound
#             cls.unbind_inputs()
#         except:
#             pass
...
# class VRUIUtils(IsaacUIUtils):
#     # dont actually need ui_window 
#     publisher=None
#     vr_controller=None
#     old_input_pos=None
#     old_input_rot=None
#     funcs=[]
#     already_pressed={}
#     names=['reset','simulation','follow target','gripper action']
#     # only needs to re-use following methods: 
#     #   _on_reload _on_reset _on_simulation_play_pause _on_follow_target_button_event _on_gripper_action
#     #   _on_logging_button_event, and _on_save_data_button_event
#     # setup, teardown methods must be overwritten, it depends on omni.ui which does not? exist in headless mode

#     # 'override' bind_inputs, _init_button_event, setUp and tearDown
#     # init/zeroing the initial pose

#     @classmethod
#     def _init_publisher(cls,vr_controller=None):
#         # destroy method from simpublisher is missing
#         world=AsyncUtils._sample._world
#         if (cls.publisher is None):
#             cls.publisher = IsaacSimPublisher(host="127.0.0.1", stage=world.stage) # for InteractiveScene
#         # THE NAME MUST BE THE SAME AS IN THE CSHARP CODE
#         if (cls.vr_controller is None):
#             cls.vr_controller=vr_controller or MetaQuest3("UnityClient")

#     @classmethod
#     async def _init_publisher_async(cls,vr_controller=None):
#         cls._init_publisher(vr_controller)
    
#     @classmethod
#     def _destroy_publisher(cls):
#         if cls.publisher is not None:
#             cls.publisher.scene_update_streamer.shutdown()
#             cls.publisher.scene_service.shutdown()
#             cls.publisher.asset_service.shutdown()
...
#     @classmethod
#     async def _destroy_publisher_async(cls):
#         cls._destroy_publisher()

#     @classmethod
#     def setUp(cls,vr_controller=None):
#         # t = threading.Thread(target=cls._loop_forever_really)
#         # t.start()
#         async def _UI_demon():
#             print("in ui_demon")
#             loop = asyncio.get_event_loop()
#             await asyncio.to_thread(partial(cls._loop_forever_really,loop))
#             # with ThreadPoolExecutor() as executor:
#             #     await asyncio.get_running_loop().run_in_executor(executor, cls._loop_forever_really)
#         async def _setUp_async():
#             cls._init_button_event()
#             cls.funcs.append(cls._default_task)
#             cls._reset_press_states()
#             if (cls.vr_controller is not None):
#                 cls.tearDown()
#             # await AsyncUtils.setUp(partial(cls._init_publisher,vr_controller))
#             await AsyncUtils.setUp_async()
#             sample = AsyncUtils._sample
#             sample._pre_physics_callback=cls._pre_physics_callback
#             # issue: isaacsim/omniverse on init launch does not populate rt_prim without out pressing simulation start first (register the timeline event)
#             # issue: w/o timeline play, the vr controller wont be registered to send back data
#             # cls.ui_elements["Simulation"].text = "TOPAUSE"
#             # await AsyncUtils._sample._world.play_async() # or reset_async()?
#             await cls._init_publisher_async()
#         asyncio.ensure_future(_setUp_async())
#         asyncio.ensure_future(_UI_demon())

#         return

#     @classmethod
#     def _loop_forever_really(cls,loop):
#         asyncio.set_event_loop(loop)
#         while True:
#             start = time.time()
#             cls.bind_inputs()
...
#     @classmethod
#     def _pre_physics_callback(cls,step_size):
#         # print(">>>adding to phy callback<<<")
#         for f in cls.funcs:
#             f()
#         del cls.funcs
#         cls.funcs=[cls._default_task]

#     @classmethod
#     def _register_zeroing_pose(cls):
#         input_data=cls.vr_controller.get_input_data()        
#         del cls.funcs
#         cls.funcs=[partial(cls._zeroing_pose,input_data)]

#     @classmethod
#     def _default_task(cls):
#         input_data=cls.vr_controller.get_input_data()
#         cls._transform_target(input_data)
#         cls._reset_press_states()
#         return

#     @classmethod
#     def _reset_press_states(cls):
#         for name in cls.names:
#             cls.already_pressed[name]=False

#     @classmethod
#     def _init_button_event(cls):
#         cls.ui_elements["Follow Target"] = button("TOSTART")
#         cls.ui_elements["Simulation"] = button("TOPLAY")
#         cls.ui_elements["Gripper Action"] = button("TOOPEN")
#         cls.ui_elements["Start Logging"] = button("START")
#         cls.ui_elements["(Re)load"] = button("(Re)load")
#         cls.ui_elements["Stop(Reset)"] = button("Stop(Reset)")
#         cls.ui_elements["Save Data"] = button("Save Data")
#         cls.ui_elements["Clear"] = button("Clear")
...
#         if cls._reset_pressed(input_data) and not cls.already_pressed["reset"]:
#             cls._reset_press_states()
#             cls.already_pressed["reset"]=True
#             print(f" reset ")
#             # TODO discard logging
#             cls._register_zeroing_pose()
#             onFinish=lambda:print("vibrate") # vibrate
#             cls._on_reset(onFinish)
#             AsyncUtils._sample._pre_physics_callback=cls._pre_physics_callback
#             return

#         if cls._simulation_pressed(input_data) and not cls.already_pressed["simulation"]:
#             cls._reset_press_states()
#             cls.already_pressed["simulation"]=True
#             val=cls._get_state_and_flip(name="Simulation",a="TOPLAY",b="TOPAUSE")
#             print(f" simulation is {val}")
#             if val:
#                 cls._register_zeroing_pose()
#             cls._on_simulation_play_pause(val)
#             async def _delayed_reset():
#                 time.sleep(0.3)
#                 cls.already_pressed["simulation"]=False
#             asyncio.ensure_future(_delayed_reset())
#             return
        
#         if cls._follow_target_pressed(input_data) and not cls.already_pressed["follow target"]:
#             cls.already_pressed["follow target"]=True
#             # TODO if a: start logging; if b: save logging
#             val=cls._get_state_and_flip(name="Follow Target",a="TOSTART",b="TOSTOP")
#             print(f" follow target is {val}")
#             if val:
#                 cls._register_zeroing_pose()
#             else:
#                 pass
#             cls._on_follow_target_button_event(val)
#             AsyncUtils._sample._pre_physics_callback=cls._pre_physics_callback
#             return
...
#                 print("vibrate: performing gripper action ")
#             cls.already_pressed["gripper action"]=True;
#             val=cls._get_state_and_flip(name="Gripper Action",a="TOOPEN",b="TOCLOSE")
#             cls._on_gripper_action(val)
...
#     @classmethod
#     def _gripper_action_pressed(cls, input_data):
#         return input_data["right"]["index_trigger"]

#     @classmethod
#     def _zeroing_pose(cls, input_data):
#         cls.old_input_pos,cls.old_input_rot = cls._get_pose(input_data)

#     @classmethod
#     def _transform_target(cls,input_data):
#         old_input_pos=cls.old_input_pos
#         old_input_rot=cls.old_input_rot
#         input_pos,input_rot= cls.old_input_pos,cls.old_input_rot = cls._get_pose(input_data)
#         # numpy is weird
#         # delta_pos=input_pos-old_input_pos
#         delta_pos=np.subtract(input_pos,old_input_pos)
#         delta_rot=mu.mul(input_rot,mu.inverse(old_input_rot)) # TODO the order is unclear in doc could be another way around
#         task_params=AsyncUtils._sample._task_params
#         old_target_pos,old_target_rot=task_params["target_position"]["value"],task_params["target_orientation"]["value"]
#         # target_pos=old_target_pos+delta_pos
#         target_pos=np.add(old_target_pos,delta_pos)
#         target_rot=mu.mul(delta_rot,old_target_rot) # TODO the order?
#         # set params expects a scalar-first (w, x, y, z) quaternion
#         # the singe_prim_wrapper no longer expect target pos/ori but delta!!! follow_target.py has to adapt
#         # ~~lul, isaac core modules are not compatible~~
#         # bad code there 
#         # AsyncUtils._sample._task.set_params(target_position=target_pos,target_orientation=target_rot)
#         # AsyncUtils._sample._task.set_params(delta_pos,delta_rot)
#         AsyncUtils._sample._target.set_local_pose(translation=target_pos, orientation=target_rot)

#         # print(f">>>tgt pos:{target_pos}    tgt rot:{target_rot}<<<")


#     @classmethod
#     def _get_pose(cls,input_data):
#         # rot should be quaternion
#         # the rot from metaquest3 class is {-rightRot.z, rightRot.x, -rightRot.y, rightRot.w} !!!!!!!
#         q_xyzw=input_data["right"]["rot"]
#         q_wxyz=np.array([q_xyzw[3],q_xyzw[0],q_xyzw[1],q_xyzw[2]])
#         pos=input_data["right"]["pos"]
...
#     def unbind_inputs(cls):
#         try:
#             _sample = AsyncUtils._sample
#             _sample._pre_physics_callback=None
#         except:
#             pass

#     @classmethod
#     def _get_state_and_flip(cls,name,a,b):
#         val=cls.ui_elements[name].text==a
#         if val:
#             cls.ui_elements[name].text=b
#         else:
#             cls.ui_elements[name].text=a
#         return val
